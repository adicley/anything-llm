...
Changes to the desktop app are downstream of the core AnythingLLM project. Releases of the desktop app are published at the same time as the core AnythingLLM project. Code from the core AnythingLLM project is copied into the desktop app into an Electron wrapper. The Electron wrapper that wraps around the core AnythingLLM project is **not** part of the core AnythingLLM project, but is maintained by the AnythingLLM team.
 ...
 Executando Testes e Pipeline de CI Localmente
Esta seção explica como configurar e executar a suíte de testes e o pipeline de CI/CD localmente antes de enviar um Pull Request.
...
PostgreSQL (ou use o container fornecido)
...
bash
yarn lint          # Verifica problemas de estilo de código
yarn format:check  # Verifica formatação
yarn format        # Aplica formatação automática
🔄 Simulando o Pipeline do GitHub Actions Localmente
Você pode usar act para executar os workflows do GitHub Actions localmente:
...
# Execute o workflow de testes
act pull_request -W .github/workflows/run-tests.yaml

# Execute o workflow de lint
act pull_request -W .github/workflows/check-lint.yaml
💡 Dicas Rápidas
Sempre execute yarn lint e yarn test antes de abrir um PR.

Use yarn format para corrigir automaticamente problemas de formatação.

Se os testes falharem, verifique se o banco de dados está em execução.

Consulte a aba Actions no GitHub para entender o fluxo do pipeline remoto.
